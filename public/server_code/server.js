...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/api/public/claim-license" && request.method === "POST") {
      ...
      // 1. Procura se o cliente já possui uma chave cadastrada no banco
      const [licenses] = await pool.execute(
        `SELECT * FROM licenses WHERE access_email=? OR account_email=? OR email=? ORDER BY created_at DESC LIMIT 1`,
        [email, email, email]
      );
      ...
      if (licenses.length > 0) {
        ...
        json(response, 200, {
          ...
          message: "Sua chave de acesso foi encontrada com sucesso!"
        });
        return;
      }

      // 2. Se não encontrou chave, verifica se existe evento pago de compra na Cakto
      const [events] = await pool.execute(
        `SELECT * FROM cakto_webhook_events WHERE customer_email=? AND (payment_status='paid' OR payment_status='approved' OR event_type='purchase_approved' OR event_type='subscription_created') ORDER BY created_at DESC LIMIT 1`,
        [email]
      );
      ...
      if (events.length > 0) {
        ...
        const plan = ev.mapped_plan || findPlanInPayload(payload) || "basic";
        ...
        await pool.execute(
          ...
          [
            ...
            "Gerado via autoatendimento do cliente",
        ...
        json(response, 200, {
          ...
          status: "pending",
          expiresAt: expiresAt,
          message: "Sua chave foi gerada e ativada com sucesso!"
        });
        return;
      }
    ...
    if (url.pathname === "/api/webhooks/cakto" && request.method === "POST") {
      ...
      if (
        ...
        console.warn(`[Webhook Cakto] FALHA DE AUTORIZAÇÃO: Secret incorreto ou não configurado. Fornecido: "${suppliedSecret}", Esperado no .env: "${CAKTO_WEBHOOK_SECRET}"`);
        json(response, 401, {
          ok: false,
      ...
      try {
        ...
        if (isPaid && event.email && plan) {
          ...
          console.log(`[Webhook Cakto] Acesso REVOGADO com SUCESSO para o email ${event.email}`);
          processed = 1;
        } else {
    ...
    if (url.pathname.startsWith("/api/admin/")) {
      const isAllowedForMod = url.pathname.startsWith("/api/admin/licenses") || 
                             url.pathname.startsWith("/api/admin/support") ||
                             url.pathname === "/api/admin/health";
