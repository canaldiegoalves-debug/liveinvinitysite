...
export default function AuthScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === 'register' && !validateCPF(cpf)) {
      setError('CPF inválido. Por favor, insira um CPF real.'); triggerShake(); return;
    }
    ...
    <div className="auth-overlay">
      ...
      <div className={`auth-card ${shake ? 'auth-card--shake' : ''}`}>
        ...
        <form className="auth-form" onSubmit={handleSubmit} noValidate>
          ...
          {isRegister && (
            <div className="auth-field">
              <label className="auth-label" htmlFor="auth-cpf">
                SEU CPF (PIX)
              </label>
              <div className="auth-input-wrapper">
                <span className="auth-input-icon">🆔</span>
          ...
          {isRegister && (
            <label className={`auth-checkbox-label ${!isAdult && error.includes('18') ? 'auth-checkbox-label--error' : ''}`}>
              ...
              <span className="auth-checkbox-text">
                ...
                e concordo com os{' '}
                e concordo com os{' '}
                <a href="/terms" target="_blank" style={{ color: '#00ff88', textDecoration: 'underline' }}>
                  Termos de Uso
                </a>{' '}
                e{' '}
                <a href="/privacy" target="_blank" style={{ color: '#00ff88', textDecoration: 'underline' }}>
                  Privacidade
                </a>
              </span>
            </label>
        ...
        <div className="auth-footer">
          ...
          <span><a href="/responsible-gaming" target="_blank" style={{ color: 'inherit', textDecoration: 'underline' }}>Jogo responsável</a> · +18 · Apenas para entretenimento</span>
        </div>
      </div>
    </div>
  );
