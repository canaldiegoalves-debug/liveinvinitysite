...
export async function refuseWithdraw(requestId: string, adminToken: string) {
  if (!adminToken) return;
  const { error } = await getSupabase()
    .from('withdraw_requests')
...
export async function createWithdrawRequest(email: string, name: string, amount: number, pixKey: string) {
  const { data: user } = await getSupabase()
    .from('profiles')
    .select('balance, total_bet, bonus_balance, total_deposited')
    .eq('email', email.toLowerCase())
    .single();
  ...
  const totalDep = Number(user.total_deposited || 0);

  if (balance < amount) return false;
  if (totalBet < 20) return false;
  const isBonusUnlocked = bonusBalance === 0 || totalDep >= (bonusBalance * 10);
  const withdrawable = isBonusUnlocked ? balance : Math.max(0, balance - bonusBalance);
  
  if (amount > withdrawable) return false;
  ...
  if (updateError) {
    // Tentativa de reverter caso a atualização do perfil falhe criticamente (ideal seria uma RPC atômica)
    await getSupabase().from('withdraw_requests').delete().eq('email', email.toLowerCase()).eq('status', 'pending');
    return false;
  }
